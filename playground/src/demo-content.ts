...
本页面展示所有可用的 x-langjs 渲染组件，每种组件提供多种用法示例。
...
\`\`\`x-langjs
agentchat(role = "agent", content = "我遇到一个分支问题：请选择接下来的操作。", questionId = "q1", options = ["继续执行", "暂停并询问", "终止流程"])
agentchat(role = "user", content = "我希望先暂停并询问你。")
agentchat(role = "agent", content = "好的，请补充一下你希望展示的字段。", questionId = "q2", options = ["字段 A/字段 B", "字段 C/字段 D"])
\`\`\`
...
以上覆盖了 **alert、statistic、progress、tag、rate、descriptions、table、button、card、ordercard、agentchat、hotelconfirm、form、result、drawer、timeline、collapse、dialog、linechart、areachart、barchart、piechart、scatterchart、candlestickchart、radarchart、graphchart** 共 26 类组件，可切换顶部组件库查看不同 UI 风格。
`
